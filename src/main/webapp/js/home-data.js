 ...
 * 限时免费 暂时写死
 */
$.get(`index?c_id=0&is_vip=1&sales=1&page=1')}`,(data)=>{
		$.each(data.data,(i,value)=>{
			if (i>=4) {return false;}
...
rankinglist.forEach(function(val, index ) {
	$.get(`/rankinglist/${val}?id=&page=1`,(data)=>{
		$.each(data.data ,(i,value)=>{
			if (i==0) {
